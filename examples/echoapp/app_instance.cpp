...
app_instance::app_instance(ULONG port, std::wstring hostname)
    : server_(nullptr), io_context_(), port_(port), hostname_(hostname), server_threads_() {
...
HRESULT STDMETHODCALLTYPE app_instance::BeginOpen( 
    ...
    belt::com::com_ptr<IFabricAsyncOperationContext> ctx = async_context::create_instance(callback).to_ptr(); 
...
HRESULT STDMETHODCALLTYPE app_instance::EndOpen( 
    ...
    belt::com::com_ptr<IFabricStringResult> addr = string_result::create_instance(addr_str).to_ptr(); 
