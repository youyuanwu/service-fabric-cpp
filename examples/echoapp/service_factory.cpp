...
HRESULT STDMETHODCALLTYPE service_factory::CreateInstance( 
    ...
    if(initializationDataLength > 0 && initializationData != nullptr){
        data = std::string(initializationData, initializationData + initializationDataLength);
    }

    BOOST_LOG_TRIVIAL(debug) << "service_factory::CreateInstance "
        << "serviceTypeName " << serviceTypeName
        << "serviceName " << serviceName
        << "initializationDataLength " << initializationDataLength
        << "initializationData" << data
        // << "partitionId " << partitionId
        << "instanceId " << instanceId;

    // create a instance and return
    belt::com::com_ptr<IFabricStatelessServiceInstance> instance = app_instance::create_instance(port_, hostname_).to_ptr(); 
