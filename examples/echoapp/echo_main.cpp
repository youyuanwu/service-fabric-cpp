...
HRESULT get_port(belt::com::ref<IFabricCodePackageActivationContext> activation_context, ULONG & port){
    const FABRIC_ENDPOINT_RESOURCE_DESCRIPTION* echoEndpoint = nullptr;

    HRESULT hr = activation_context->GetServiceEndpointResource(L"ServiceEndpoint1", &echoEndpoint);
    if(hr != S_OK){
        BOOST_LOG_TRIVIAL(error) << "GetServiceEndpointResource failed: " << hr << " " << sf::get_fabric_error_str(hr);
        return hr;
    }
    BOOST_LOG_TRIVIAL(error) <<"name: " << echoEndpoint->Name << " port: " << echoEndpoint->Port
        << " protocol: "<< echoEndpoint->Protocol << " type: " << echoEndpoint->Type << " cert: " << echoEndpoint->CertificateName;
    port = echoEndpoint->Port;
    return S_OK;
}

HRESULT get_hostname(belt::com::ref<IFabricCodePackageActivationContext> activation_context, std::wstring & hostname){
    // get node ip
    ...
    if(hr != S_OK){
        BOOST_LOG_TRIVIAL(error) << "FabricBeginGetNodeContext failed: " << sf::get_fabric_error_str(hr);
        return hr;
    }
    callback->Wait();
    ...
    if(hr != S_OK){
        BOOST_LOG_TRIVIAL(error) << "FabricEndGetNodeContext failed: " << sf::get_fabric_error_str(hr);
        return hr;
    }
    ...
    BOOST_LOG_TRIVIAL(error) << "Node Ctx info/hostname:" << node_ctx_res->IPAddressOrFQDN;
    
    hostname = node_ctx_res->IPAddressOrFQDN;
    return S_OK;
}

HRESULT run_app(belt::com::ref<IFabricRuntime> fabric_runtime,
    ...
    return fabric_runtime->RegisterStatelessServiceFactory(L"EchoAppService", service_factory.detach());
}
...
int main(int argc, char* argv[]) {
    ...
    HRESULT hr = ::FabricCreateRuntime(IID_IFabricRuntime, (void**)fabric_runtime.put());
    if(hr != NO_ERROR){
        BOOST_LOG_TRIVIAL(error) << "FabricCreateRuntime failed: " << hr << " " << sf::get_fabric_error_str(hr);
        return EXIT_FAILURE;
    }

    hr = ::FabricGetActivationContext(IID_IFabricCodePackageActivationContext, (void**)activation_context.put());
    if(hr != NO_ERROR){
