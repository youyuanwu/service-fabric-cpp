...
HRESULT commit_helper(winrt::com_ptr<IFabricTransaction> &tx) {
  ...
  if (hr != S_OK) {
    spdlog::error("cannot BeginCommit: {}", hr);
    return hr;
  }
  callback->Wait();
  ...
  if (hr != S_OK) {
    spdlog::error("cannot EndCommit: {}", hr);
    return hr;
  }
  return S_OK;
}
...
public:
  ...
  HRESULT add(std::wstring key, std::string val) {
    ...
    if (hr != S_OK) {
      spdlog::error("cannot open tx: {}", hr);
      return hr;
    }
    ...
    if (b) {
      spdlog::info(L"key already exist: {} {}", key, hr);
      tx->Rollback();
      return E_FAIL;
    }
    ...
    hr = commit_helper(tx);
    if (hr != S_OK) {
      spdlog::error("commit_helper failed: {}", hr);
      tx->Rollback();
      return hr;
    }
    return S_OK;
  ...
  HRESULT get(std::wstring key, std::string &val) {
    ...
    if (!b) {
      spdlog::info(L"key does not exist: {} {}", key, hr);
      tx->Rollback();
      return E_FAIL;
    }
    ...
    if (hr != S_OK) {
      spdlog::error("get_helper failed: {}", hr);
      tx->Rollback();
      return hr;
    }
  ...
  HRESULT put(std::wstring key, std::string val) {
    ...
    if (hr != S_OK) {
      spdlog::error("put_helper failed: {}", hr);
      tx->Rollback();
      return hr;
    }
    return S_OK;
...
private:
  ...
  HRESULT contains_key(winrt::com_ptr<IFabricTransaction> &tx,
    ...
    if (hr != S_OK) {
      spdlog::error("cannot do contains: {}", hr);
      return hr;
    }
    ret = b;
    return S_OK;
