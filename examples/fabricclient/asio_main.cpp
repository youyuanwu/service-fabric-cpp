...
int main(){
    ...
    auto lamda_callback = [client](IFabricAsyncOperationContext* ctx){
        ...
        if(hr != NO_ERROR){
            BOOST_LOG_TRIVIAL(debug) << "EndGetNodeList failed: "<< hr << " " << sf::get_fabric_error_str(hr);
            return;
        }
    ...
    belt::com::com_ptr<IFabricAsyncOperationCallback> callback = 
        sf::AsioCallback::create_instance(lamda_callback, io_context.get_executor()).to_ptr();

    belt::com::com_ptr<IFabricAsyncOperationContext> ctx;
    FABRIC_NODE_QUERY_DESCRIPTION node = {};
    hr = client->BeginGetNodeList(&node, 1000, callback.get(), ctx.put());
    if(hr != NO_ERROR){
        BOOST_LOG_TRIVIAL(debug) << "BeginGetNodeList failed: " << hr << sf::get_fabric_error_str(hr);
        return EXIT_FAILURE;
    }
