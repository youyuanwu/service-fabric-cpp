...
namespace Common {
template <typename T, typename D = std::default_delete<T>> class MoveUPtr {
...
public:
  ...
  std::unique_ptr<T, D> TakeUPtr() { return std::move(this->ptr_); }

  bool HasPtr() { return (this->ptr_.get() != nullptr); }
