...
protected:
  ...
  // Adds a reference if the current ref-count is >0 and is no-op if the
  // ref-count is 0. This can be safely called concurrently with the COM object
  // destructor.
  //
  // Returns the current ref-count, which will be 0 iff the ref-count was
  // already 0. If this returns 0 the caller MUST NOT call Release, and if this
  // returns non-zero the caller MUST call Release.
  //
  // This enables a COM object to fire events safely where the event state
  // includes a reference to the COM object.  The COM object needs to be allowed
  // to go to 0 ref count, but at the same time when a new event is getting
  // ready to fire, it needs to add a ref count.  To do this safely, two
  // guarantees must be met: 1) The destructor of the COM object must coordinate
  // to ensure that no more events attempt
  //    to use the COM object after the destructor has run.
  // 2) Events must avoid adding a reference if the ref count is already 0.
  // This method helps guarantee #2.  It is up to the COM object implementation
  // to guarantee #1.
...
//
template <class TComImplementation> ComPointer<TComImplementation> make_com() {
...
ComPointer<TComImplementation> make_com(T0 &&a0, T1 &&a1) {
  ComPointer<TComImplementation> p;
  p.SetNoAddRef(
      new TComImplementation(std::forward<T0>(a0), std::forward<T1>(a1)));
  return p;
}
...
template <class TComImplementation, class ComInterface, class T0, class T1,
          class T2, class T3, class T4, class T5, class T6>
ComPointer<ComInterface> make_com(T0 &&a0, T1 &&a1, T2 &&a2, T3 &&a3, T4 &&a4,
                                  T5 &&a5, T6 &&a6) {
  ComPointer<ComInterface> p;
  p.SetNoAddRef(new TComImplementation(
...
//
// A set of macros to define the list of interfaces supported by QueryInterface.
// Typical usage goes at the beginning of a class (before the first access
// specifier) and lists out all the interfaces.  IUnknown should typically be
// first.
//
// BEGIN_COM_INTERFACE_LIST must come first, and specifies the implementation
// class.
//
// Each COM_INTERFACE_ITEM specifies an IID and a type that either is or
// uniquely derives from the corresponding interface type.  For example, in
// cases where a class implements multiple interfaces, each of which extends
// IUnknown, the IID_IUnknown entry needs to specify exactly one of the
// interfaces to choose which IUnknown pointer is returned for IID_IUnknown.
//
// COM_BREAK_IN_DEBUGGER_INTERFACE_ITEM behaves similarly to COM_INTERFACE_ITEM,
// except it calls DebugBreak when someone queries for the specified interface
// before returning it.
//
...
// Here is an example:
//
//    class ComOrange : public IOrange, public IFruit, public IColor, private
//    ComUnknownBase
//    {
//        DENY_COPY(ComOrange)
//
...
#define COM_INTERFACE_ITEM(iid, TInterface)                                    \
  if (riid == (iid)) {                                                         \
    *result = static_cast<TInterface *>(this);                                 \
  } else
...
//
// Shortcut macro to define an implementation of IUnknown with only one
...
//
// Shortcut macro to define an implementation of IUnknown with one specialized
// interface and delegate the rest to TBase's QueryInterface.
//
//   TClass     - name of the class in which this macro is used
//   iid        - IID of TInterface
//   TInterface - name of the COM interface this class exposes
...
//
// Shortcut macro to define an implemenation of IUnknown with two specialized
// interfaces.  If ambiguous, the object's IUnknown comes from the last
// interface provided.
//
//   TClass      - name of the class in which this macro is used
