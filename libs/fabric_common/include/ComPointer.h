...
public:
  ...
  // Construction from raw pointer is intentionally disallowed because it is not
  // clear whether the caller wants ComPointer to AddRef or not.  To make this
  // explicit, we require the caller to construct an empty ComPointer and call
  // SetNoAddRef or SetAndAddRef explicitly.
  //
  // However, note that there is a constructor that does QI.  Because the
  // lifetime model for QI is explicit this is safe.

  explicit ComPointer() throw() : p_(NULL) {}

  ComPointer(ComPointer<TComInterface> const &other) throw() : p_(other.p_) {
    if (p_) {
      p_->AddRef();
    }
  }

  ComPointer(ComPointer<TComInterface> &&other) throw() : p_(other.p_) {
    other.p_ = NULL;
  }

  // Supports "casting" a smart-pointer via QueryInterface.  This intentionally
  // does not make riid an optional parameter so that the mandatory REFIID
  // parameter gives a hint to the caller that a QueryInterface is occurring.
  ...
  ComPointer(ComPointer<TOtherComInterface> const &other, REFIID riid) throw()
    ...
    if (FAILED(other->QueryInterface(riid, reinterpret_cast<void **>(&p_)))) {
      // Correct implementations of QI will always set this NULL, but there are
      // many incorrect implementations of QI in the world.
      p_ = NULL;
    }
  }

  // Similar to previous constructor, but uses IUnknown*.
  ComPointer(IUnknown *other, REFIID riid) throw() : p_(NULL) {
  ...
  bool operator==(ComPointer const &other) const throw() {
    return p_ == other.p_;
  }
  bool operator!=(ComPointer const &other) const throw() {
    return p_ != other.p_;
  ...
  ComPointer<TComInterface> const &
  operator=(ComPointer<TComInterface> const &other) throw() {
    if (p_ != other.p_) {
  ...
  ComPointer<TComInterface> const &
  operator=(ComPointer<TComInterface> &&other) throw() {
    if (this != &other) {
      ComPointer<TComInterface> cleanup(std::move(*this));
      p_ = other.p_;
      other.p_ = NULL;
  ...
  template <class TOtherComInterface>
  bool IsSameComObject(ComPointer<TOtherComInterface> const &other) const
      throw() {
    if (!(*this)) {
      return !other;
    } else if (!other) {
  ...
  TComInterface *GetRawPointer() const throw() { return p_; }
...
public: // keep intellisense happy by moving this to the bottom
  // OPERATOR_BOOL
  operator bool() explicit const throw() { return (p_ != NULL); }
};
} // namespace Common
