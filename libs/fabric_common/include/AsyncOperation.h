...
// AsyncCallback type represents a pointer to the function that is called when
// an AsyncOperation is completed.
...
struct OperationContext : public OperationContextBase {
  ...
  OperationContext(TContext &&context) : Context(std::move(context)) {}
};

// ************************************************************************************************************
// AsyncOperation:
//  This is a base type that represents an asynchronous operation. This type is
//  used by derivation by the asynchronous operation writers. It contains enough
//  information to execute the operation asynchronously and return results to
...
public:
  ...
  inline std::wstring const &get_AsyncOperationTraceId() const {
    return traceId_;
  }

  inline ErrorCode const &get_Error() const { return error_; }

  inline AsyncOperationSPtr const &get_State() const { return parent_; }

  inline bool get_IsCompleted() const { return state_.IsCompleted; }
  ...
  // This method is called to indicate an intent to complete this
  // AsyncOperation. It returns true if the operation was not completed and
  // returns false if the operation was already completed or was in the process
  // of completion. Once TryStartComplete returns true FinishComplete must be
  // called to supply the error code and invoke completion callback.
  ...
  void FinishComplete(AsyncOperationSPtr const &thisSPtr,
                      ErrorCode const &error = ErrorCode());
  void FinishComplete(AsyncOperationSPtr const &thisSPtr, ErrorCode &&error);

  // This method will try to complete this AsyncOperation with the specified
  // error. The AsyncOperation must be started. It returns true if the
  // AsyncOperation was completed successfully, otherwise false.
  bool TryComplete(AsyncOperationSPtr const &thisSPtr,
                   ErrorCode const &error = ErrorCode());
  ...
  // will succeed, as it may be called too late (as the operation is already
  // completing).
  void Cancel(bool forceComplete = false);

  template <typename TContext>
  void
  PushOperationContext(std::unique_ptr<OperationContext<TContext>> &&context) {
  ...
  std::unique_ptr<OperationContext<TContext>> PopOperationContext() {
    ...
    OperationContext<TContext> *result =
        dynamic_cast<OperationContext<TContext> *>(contexts_.release());
#else
    OperationContext<TContext> *result =
        static_cast<OperationContext<TContext> *>(contexts_.release());
#endif
    if (result) {
  ...
  static AsyncOperationSPtr CreateAndStart(Args &&...args) {
    AsyncOperationSPtr asyncOperation =
        std::make_shared<TAsyncOperation>(std::forward<Args>(args)...);
    asyncOperation->Start(asyncOperation);
...
#if DBG && !defined(PLATFORM_UNIX)
    auto tAsyncOperation =
        dynamic_cast<TAsyncOperation *>(asyncOperation.get());
#else
    auto tAsyncOperation = static_cast<TAsyncOperation *>(asyncOperation.get());
#endif

    // ASSERT_IF(
    //     tAsyncOperation == nullptr,
    //     "AsyncOperation<TAsyncOperation>::Get(...) tAsyncOperation ==
    //     nullptr");
    assert(!(tAsyncOperation == nullptr));
  ...
  static TAsyncOperation *End(AsyncOperationSPtr const &asyncOperation) {
    ...
    // ASSERT_IF(asyncOperation->contexts_ != nullptr, "Context not empty when
    // operation ended");
    assert(!(asyncOperation->contexts_ != nullptr));
    asyncOperation->state_.TransitionEnded();
    return tAsyncOperation;
  }
...
protected:
  ...
  inline bool get_InternalIsCompleted() const {
    return state_.InternalIsCompleted;
  }
  ...
  // The derive type override this method to implement the logic of the
  // asynchronous operation. This method will be called once, immediately after
  // the construction of the OperationContext
...
public:
  ...
  static AsyncOperationSPtr Create(TAsyncOperationRoot const &root) {
    return std::static_pointer_cast<AsyncOperation>(
        std::make_shared<AsyncOperationRoot<TAsyncOperationRoot>>(root));
  }

  static AsyncOperationSPtr Create(TAsyncOperationRoot &&root) {
    return std::static_pointer_cast<AsyncOperation>(
        std::make_shared<AsyncOperationRoot<TAsyncOperationRoot>>(
            std::move(root)));
  }

  static TAsyncOperationRoot &Get(AsyncOperationSPtr const &parent) {
    auto rootAsyncOperation =
        static_cast<AsyncOperationRoot<TAsyncOperationRoot> *>(parent.get());
    // ASSERT_IF(rootAsyncOperation == nullptr,
    //           "AsyncOperation<TAsyncOperationRoot>::GetRoot(...) "
    //           "rootAsyncOperation == nullptr");
    assert(rootAsyncOperation);
...
template <typename TContext> class OperationContextCallback {
public:
