...
public:
  ...
  // static std::string AddField(Common::TraceEvent & traceEvent, std::string
  // const & name); void FillEventData(Common::TraceEventContext & context)
  // const;
...
private:
  ...
  // This is set to true whenever we initialize or assign this from another
  // ErrorCode. If we initialize from an explicit ErrorCodeValue we do not need
  // to check since the code already knows the value.
  //
...
public:
  ...
  void Update(ErrorCode newValue) {
    InterlockedCompareExchange(reinterpret_cast<LONG volatile *>(&value_),
                               newValue.ReadValue(), ErrorCodeValue::Success);
  }
