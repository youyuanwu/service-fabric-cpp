...
namespace Common {
// const StringLiteral
// TraceComponent_ComAsyncOperationContext("ComAsyncOperationContext");

ComAsyncOperationContext::ComAsyncOperationContext(bool skipCompleteOnCancel)
    : ComUnknownBase(), state_(), callback_(), result_(ErrorCodeValue::Success),
      childLock_(), child_(), rootSPtr_(),
...
ComAsyncOperationContext::GetCPtr(AsyncOperationSPtr const &proxySPtr) {
  return AsyncOperationRoot<ComAsyncOperationContextCPtr>::Get(proxySPtr);
}

HRESULT
ComAsyncOperationContext::Start(ComAsyncOperationContextCPtr const &thisCPtr) {
  CheckThisCPtr(thisCPtr);
  state_.TransitionStarted();

  AsyncOperationSPtr proxySPtr =
      AsyncOperationRoot<ComAsyncOperationContextCPtr>::Create(thisCPtr);
  ...
  // cannot start and finish between the check and setting the child.
  {
    ...
    if (state_.InternalIsCompletingOrCompleted ||
        state_.InternalIsCancelRequested) {
      cancel = std::move(proxySPtr);
    }

    // Moves child if we did not already move to cancel, otherwise clears
    // child_.
...
void ComAsyncOperationContext::FinishComplete(
    AsyncOperationSPtr const &proxySPtr, HRESULT result) {
  FinishComplete(GetCPtr(proxySPtr), result);
}

void ComAsyncOperationContext::FinishComplete(
    ComAsyncOperationContextCPtr const &thisCPtr, HRESULT result) {
  FinishComplete(thisCPtr, ErrorCode::FromHResult(result));
}

void ComAsyncOperationContext::FinishComplete(
    ComAsyncOperationContextCPtr const &thisCPtr, ErrorCode const &error) {
  CheckThisCPtr(thisCPtr);
...
bool ComAsyncOperationContext::TryComplete(AsyncOperationSPtr const &proxySPtr,
                                           HRESULT result) {
