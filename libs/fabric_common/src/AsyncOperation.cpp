...
AsyncOperation::AsyncOperation()
    : enable_shared_from_this<AsyncOperation>(), state_(), callback_(nullptr),
      parent_(), error_(ErrorCode()), childLock_(), children_(),
...
AsyncOperation::AsyncOperation(AsyncCallback const &callback,
    ...
    : enable_shared_from_this<AsyncOperation>(), state_(), callback_(callback),
      parent_(parent), error_(ErrorCode()), childLock_(), children_(),
...
void AsyncOperation::Start(AsyncOperationSPtr const &thisSPtr) {
  // ASSERT_IF(thisSPtr.get() != this, "AsyncOperation::Start thisSPtr.get() !=
  // this");
  assert(thisSPtr.get() == this);
  ...
  // ALWAYS Call OnStart() before attempting to cancel the operation as it is a
  // contract many components depend on
  OnStart(thisSPtr);
  ...
  if (attachToParentSucceeded == false) {
    ...
    // One of the usage pattern is to CreateAndStart async operation under a
    // lock, with the expectation that no work is done in Start to take that
    // lock. When the operation is cancelled, it is completed, and OnComplete
    // may take the same lock to change state. To prevent any deadlocks, post
    // Cancel so it executes outside the lock. A better fix is tracked through
    // SQL Server: 1209734: Change AsyncOperation Start to return ErrorCode
...
void AsyncOperation::FinishComplete(AsyncOperationSPtr const &thisSPtr,
...
void AsyncOperation::FinishCompleteInternal(
    AsyncOperationSPtr const &thisSPtr) {
  // ASSERT_IF(thisSPtr.get() != this, "AsyncOperation::FinishComplete
  // thisSPtr.get() != this");
  assert(thisSPtr.get() == this);
  // Trace.WriteNoise(
  //     TraceType_AsyncOperation,
...
bool AsyncOperation::TryComplete(AsyncOperationSPtr const &thisSPtr,
                                 ErrorCode &&error) {
  if (TryStartComplete()) {
...
bool AsyncOperation::AttachChild(AsyncOperationSPtr &&child) {
...

  {
    ...
    // If there is a racing Cancel call, we need to ensure that either we see
    // that this is canceled, or Cancel sees the child attached.
    //
    // Checking canceled under the lock guarantees that a non-trivial Cancel()
    // cannot start and finish between the check and setting the child.
  ...
  //     TraceType_AsyncOperation,
  //     traceId_,
  //     "Not attaching child AsyncOperation {0} as parent in
  //     completed/completing/cancelling.", wformatString("{0}",
