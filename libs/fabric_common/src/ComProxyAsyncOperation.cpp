...
class ComProxyAsyncOperation::ComAsyncOperationCallback
  ...
  COM_INTERFACE_LIST1(ComProxyAsyncOperation, IID_IFabricAsyncOperationCallback,
                      IFabricAsyncOperationCallback)

public:
  ComAsyncOperationCallback(ComProxyAsyncOperation &owner,
  ...
  virtual void STDMETHODCALLTYPE ComAsyncOperationCallback::Invoke(
    ...
    // ASSERT_IF(CommonConfig::GetConfig().MultipleAsyncCallbackInvokeAssertEnabled
    // && previous, "async callback has been invoked multiple times");
    //  do not support invoke mutiple times
    assert(!previous);
...
ComProxyAsyncOperation::ComProxyAsyncOperation(AsyncCallback const &callback,
                                               AsyncOperationSPtr const &parent,
                                               bool skipCompleteOnCancel)
    : AsyncOperation(callback, parent, skipCompleteOnCancel), lock_(),
...
void ComProxyAsyncOperation::OnStart(AsyncOperationSPtr const &thisSPtr) {
  ComPointer<IFabricAsyncOperationCallback> callback =
      make_com<ComAsyncOperationCallback, IFabricAsyncOperationCallback>(
          *this, thisSPtr);

  ComPointer<IFabricAsyncOperationContext> cleanup;
  HRESULT hr = BeginComAsyncOperation(callback.GetRawPointer(),
  ...
  if (cleanup->CompletedSynchronously()) {
    ...
    if (!this->InternalIsCompleted) {
      comAsyncOperationSPtr_ =
          std::make_shared<ComPointer<IFabricAsyncOperationContext>>(
              std::move(cleanup));
    }
  }
}
...
HRESULT ComProxyAsyncOperation::OnComAsyncOperationCompleted(
    AsyncOperationSPtr const &thisSPtr, IFabricAsyncOperationContext *context) {
  HRESULT hr = S_OK;
...
HRESULT
ComProxyAsyncOperation::TryComplete_HRESULT(AsyncOperationSPtr const &thisSPtr,
                                            HRESULT hr,
                                            bool captureThreadErrorMessage) {
  TryComplete(thisSPtr,
              std::move(ErrorCode::FromHResult(hr, captureThreadErrorMessage)));
