...
// ComUtility::ComUtilitySource("Common.ComUtility");

namespace {
typedef HRESULT(WINAPI *CoGetApartmentTypeFPtr)(
    __out APTTYPE *pAptType, __out APTTYPEQUALIFIER *pAptQualifier);

typedef HRESULT(WINAPI *SetErrorInfoFPtr)(__in DWORD dwReserved,
...
bool ComUtility::Initialize() {
  ...
  if (ole32Module == NULL) {
    BOOL getModuleResult = ::GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_PIN,
                                                L"ole32.dll", &ole32Module);
    if (!getModuleResult) {
      return false;
    }
  }
  ...
  if (coGetApartmentTypeFPtr == NULL) {
    FARPROC rawCoGetApartmentTypeFPtr =
        ::GetProcAddress(ole32Module, "CoGetApartmentType");
    if (!rawCoGetApartmentTypeFPtr) {
      return false;
    }
    coGetApartmentTypeFPtr =
        reinterpret_cast<CoGetApartmentTypeFPtr>(rawCoGetApartmentTypeFPtr);
  }
...
ErrorCode ComUtility::CheckMTA() {
  ...
  HRESULT getApartmentTypeResult =
      (*coGetApartmentTypeFPtr)(&apartmentType, &apartmentTypeQualifier);
  ...
  case APTTYPE_NA:
    ...
    //     "COM apartment state must be uninitialized or MTA.  Current apartment
    //     state is {0}.", static_cast<int>(apartmentType));
    return ErrorCode(
        ErrorCodeValue::InvalidState); // TODO: return correct error code
...
// CLR sets ErrorInfo to an Exception object to propagate Exceptions through
// COM.
//
// Unfortunately, the thread may be returning from COM with an Exception from an
// AppDomain that has been unloaded.  This results in the user seeing an
// AppDomainUnloadedException where they were expecting a FabricException.
//
// To work around this, we clear error info whenever we return a failure
// HRESULT.
...
HRESULT ComUtility::OnPublicApiReturn(Common::ErrorCode const &error) {
  return OnPublicApiReturn(error.ToHResult(), std::wstring(error.Message));
}
