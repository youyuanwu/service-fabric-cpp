...
ErrorCode::ErrorCode()
    : value_(ErrorCodeValue::Success), message_(),
      messageTid_(UninitializedTid), read_(false),
      enableOverwriteCheck_(false) {}

ErrorCode::ErrorCode(ErrorCode const &other)
    : value_(other.ReadValue()), message_(other.message_),
      messageTid_(other.messageTid_), read_(false),
      enableOverwriteCheck_(false) {}

ErrorCode::ErrorCode(ErrorCode &&other)
    : value_(other.ReadValue()), message_(move(other.message_)),
      messageTid_(move(other.messageTid_)), read_(false),
      enableOverwriteCheck_(false) {}

ErrorCode::ErrorCode(ErrorCodeValue::Enum value)
    : value_(value), message_(), messageTid_(UninitializedTid), read_(false),
      enableOverwriteCheck_(false) {}

ErrorCode::ErrorCode(ErrorCodeValue::Enum value, wstring &&msg)
    : value_(value), message_(move(msg)), messageTid_(UninitializedTid),
      read_(false), enableOverwriteCheck_(false) {}

ErrorCode::~ErrorCode() {
  // internal fabric code. safe to ignore.
  // if (messageTid_ != UninitializedTid)
  // {
  //     auto hr = ::FabricClearLastErrorMessage(messageTid_);
  //     if (FAILED(hr))
  //     {
  //         Trace.WriteWarning(TraceComponent,
  //         "~FabricClearLastErrorMessage({0}) failed: hr = {1}", messageTid_,
  //         hr);
  //     }
  // }
}
...
bool ErrorCode::IsWin32Error(DWORD win32Error) const {
  return ReadValue() ==
         static_cast<ErrorCodeValue::Enum>(HRESULT_FROM_WIN32(win32Error));
}

bool ErrorCode::IsNtStatus(NTSTATUS status) const {
  return ReadValue() ==
         static_cast<ErrorCodeValue::Enum>(HRESULT_FROM_NT(status));
}
...
void ErrorCode::SetThreadErrorMessage() const {
  if (value_ != ErrorCodeValue::Success && !message_.empty()) {
    ...
    // if (FAILED(hr))
    // {
    //     Trace.WriteWarning(TraceComponent, "FabricSetLastErrorMessage failed:
    //     hr = {0}", hr);
    // }
  }
}
...
wstring ErrorCode::GetThreadErrorMessage(bool clear) {
  ...
  //         {
  //             Trace.WriteWarning(TraceComponent,
  //             "FabricClearLastErrorMessage, failed: hr = {0}", hr);
  //         }
  //     }

  //     hr = StringUtility::LpcwstrToWstring(stringCPtr->get_String(), true,
  //     result); if (FAILED(hr))
  //     {
  //         Trace.WriteWarning(TraceComponent, "FabricGetLastErrorMessage,
  //         LPCWSTR->wstring failed: hr = {0}", hr);
  //     }
  // }
  // else
  // {
  //     Trace.WriteWarning(TraceComponent, "FabricGetLastErrorMessage failed:
  //     hr = {0}", hr);
  // }
...
// void ErrorCode::WriteTo(Common::TextWriter & w, Common::FormatOptions const
// &) const
// {
//     w << value_;
// }
...
ErrorCode ErrorCode::FromHResult(HRESULT hr) { return FromHResult(hr, false); }
...
ErrorCode ErrorCode::FromErrno() { return ErrorCode::FromErrno(errno); }

ErrorCode ErrorCode::FromErrno(int error) {
  // LINUXTODO consider removing HRESULT_FROM_WIN32 below (revert back to commit
  // fab0ac79d978d646454ca13561154f031b5008c8), so that ErrorCode generated from
  // PAL win32 error will not accidentally collide with ErrorCode generated from
  // errno. This will require all code call FromWin32Error instead of directly
  // calling FromHResult(HRESULT_FROM_WIN32(error)).
  return FromHResult(HRESULT_FROM_WIN32(error));
}

bool ErrorCode::IsErrno(int error) const {
  return ReadValue() ==
         static_cast<ErrorCodeValue::Enum>(HRESULT_FROM_WIN32(error));
}
...
//     if (error.IsSuccess())
//     {
//         TraceNoise(taskCode, traceType, traceId,  "Operation:{0},
//         Result={1}", operation, error);
//     }
//     else
//     {
//         TraceWarning(taskCode, traceType, traceId,  "Operation:{0},
//         Result={1}", operation, error);
//     }
...
//     if (error.IsSuccess())
//     {
//         TraceNoise(taskCode, traceType, "Operation:{0}, Result={1}",
//         operation, error);
//     }
//     else
//     {
//         TraceWarning(taskCode, traceType, "Operation:{0}, Result={1}",
//         operation, error);
//     }
...
bool ErrorCode::TryAssignFrom(ErrorCode const &other, bool allowOverwrite) {
  if (this != &other) {
    // Read other value before asserting.  This prevents other from asserting in
    // cases where we assert.
    //
    auto otherValue = other.ReadValue();

    if (allowOverwrite) {
      if ((value_ != ErrorCodeValue::Success) &&
          (otherValue == ErrorCodeValue::Success)) {
        // Assert::CodingError("Cannot overwrite error value {0} with Success.",
        // value_);
        assert(false);
      }
    } else {
      // ASSERT_IF(enableOverwriteCheck_ && !read_, "ErrorCode {0} was not
      // read", value_);
      assert(!(enableOverwriteCheck_ && !read_));
    }
...
// std::string ErrorCode::AddField(Common::TraceEvent & traceEvent, std::string
// const & name)
// {
//     std::string format = "{0}";
//     size_t index = 0;

//     traceEvent.AddEventField<Common::ErrorCodeValue::Trace>(format, name +
//     ".val", index);
...
FirstErrorTracker::FirstErrorTracker() : value_(ErrorCodeValue::Success) {}
