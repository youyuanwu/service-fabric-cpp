...
public:
  ComponentRootReference(ComponentRootSPtr &&root, uint64 id,
...
ComponentRoot::ComponentRoot(bool enableReferenceTracking)
    : enable_shared_from_this(), traceId_(), referenceTracker_() {
...
ComponentRoot::ComponentRoot(ComponentRoot const &other)
    : enable_shared_from_this(other), traceId_(other.traceId_),
      referenceTracker_()
//, leakDetectionTimer_()
{}

ComponentRoot::ComponentRoot(ComponentRoot &&other)
    : enable_shared_from_this(other), traceId_(move(other.traceId_)),
      referenceTracker_()
//, leakDetectionTimer_()
{}
...
ComponentRoot::~ComponentRoot() {
  ...
  // {
  //     Trace.WriteInfo(ComponentRootTraceType, "{0} Cancelling leak detection
  //     timer", this->TraceId);
...
AsyncOperationSPtr ComponentRoot::CreateAsyncOperationRoot() const {
  return AsyncOperationRoot<ComponentRootSPtr>::Create(
      this->CreateComponentRoot());
}

AsyncOperationSPtr ComponentRoot::CreateAsyncOperationMultiRoot(
    std::vector<ComponentRootSPtr> &&roots) {
  return AsyncOperationRoot<std::vector<ComponentRootSPtr>>::Create(
      std::move(roots));
}
...
//     if (referenceTracker_.get() == nullptr)
//     {
//         Trace.WriteInfo(ComponentRootTraceType, "{0}
//         TryStartLeakDetectionTimer: Reference tracking is disabled",
//         this->TraceId);
//     }
//     else if (leakDetectionTimer_.get() == nullptr)
//     {
//         ComponentRootWPtr weakRoot = this->CreateComponentRoot();
//         leakDetectionTimer_ = Timer::Create(ComponentRootTraceType,
//         [weakRoot] (TimerSPtr const &) {
//         LeakDetectionTimerCallback(weakRoot); });

//         Trace.WriteInfo(ComponentRootTraceType, "{0} Scheduling leak
//         detection timer: {1}", this->TraceId, timeout);
