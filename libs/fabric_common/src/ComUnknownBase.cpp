...
namespace Common {
ComUnknownBase::ComUnknownBase() : refCount_(1), isShared_(false) {}

ComUnknownBase::~ComUnknownBase() {
  // ASSERT_IF(refCount_ != 0, "ComUnknownBase: Destructed with non-zero
  // ref-count.");
  assert(refCount_ == 0);
}
...
ULONG ComUnknownBase::BaseRelease(void) {
  ...
  // ASSERT_IF(refCount == MinusOneULONG, "ComUnknownBase: Too many calls to
  // Release.");
  assert(refCount != MinusOneULONG);
