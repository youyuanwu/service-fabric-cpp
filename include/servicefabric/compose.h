...
class compose_op{
public:
    compose_op(Executor ex, belt::com::com_ptr<Client> client, BeginFunc bf, EndFunc ef)
    : ex_(ex), client_(client), bf_(bf), ef_(ef),
        callback_(), ctx_() {
    ...
    void async_exec(Query * q, std::function<void(boost::system::error_code, Result*)> token){
        ...
        if(hr != NO_ERROR){
            auto ec = boost::system::error_code(hr,
                                     boost::asio::error::get_system_category());
