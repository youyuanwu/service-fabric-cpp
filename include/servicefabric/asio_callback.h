...
public:
    ...
    callback_obj(std::function<void(IFabricAsyncOperationContext*)> token, Executor ex)
    ...
    // set the reference
    void set_ctx(IFabricAsyncOperationContext *context){
        ctx_ = context;
    }
...
public:
    template<typename Executor>
    AsioCallback(std::function<void(IFabricAsyncOperationContext*)> token, Executor ex)
        : c_obj_(token, ex){}
